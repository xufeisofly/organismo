RSpec.describe Organismo::Document do
  ...
  describe 'example' do
    ...
    it 'multiple examples' do
      ...
      expect(html).to eq "<pre>   这么做有两个前提，</pre><pre> 一个是有很多地方用到了对customer是否为空的判断，\n   另一个是null类型的customer和正常的customer大部分行为保持一致</pre>"
    end
  end
  ...
  describe 'code' do
    it '#+SRC to pre' do
      ...
      expect(html).to eq "<pre>   result += &quot;&lt;h1&gt;\#{name}&lt;/h1&gt;&quot;\n     end\n   end</pre>"
    end
  ...
  describe 'org combination to html' do
    it 'combination text' do
      ...
      expect(html).to eq "<p>重构技巧之简化条件表达式</p><p></p><h2>Decompose Conditional</h2><blockquote><p>   分解条件表达式</p></blockquote><pre>   if date.before(START_DATE) || date.after(END_DATE) then\n     do_something\n   end</pre><p>   将条件中的表达式提取出来，可以大大降低复杂程度</p><pre>   def not_proper_time\n     date.before(START_DATE) || date.after(END_DATE)\n   end</pre><pre>   有时候尽管条件代码很短，但代码意图和代码自身之间往往存在着不小的差距</pre><p>   </p><h2>Consolidate Conditional Expression</h2><blockquote><p>   合并条件表达式</p><br><p>   当几种条件执行的行为都一样，则可以合并成一种条件</p></blockquote><p></p><h2>Introduce Null Object</h2><blockquote><p>   引入Null对象，可以删除条件语句</p></blockquote><p>   只有大多数代码都要求空对象做出相同响应的时，使用Null对象才有意义</p><p>   比如</p><pre>   if deal.customer.blank? then\n     customer_name = &#39;occupant&#39;\n   else\n     customer_name = customer.name\n   end</pre><p>   就可以单独建一个Null类</p><pre>   class Customer::Null\n     def name\n       &#39;occupant&#39;\n     end\n   end\n\n   class Customer::Normal\n     def name\n       super\n     end\n   end\n   \n   class Deal\n     def customer\n       super.blank ? Customer::Null.new : Customer::Normal.new\n     end\n   end</pre><p>   这样就可以去掉条件语句，变成</p><pre>   customer_name = customer.name</pre><pre>   这么做有两个前提，\n   一个是有很多地方用到了对customer是否为空的判断，\n   另一个是null类型的customer和正常的customer大部分行为保持一致</pre><p></p><h2>Introduce Assertion</h2><blockquote><p>   引入断言</p></blockquote><p>   这个ruby貌似没有必要</p><p>   什么是断言，就是断言某个条件一定成立，否则抛出异常，没有必要用它，因为：</p><p>   1. 可以使用测试代码，健壮程序，使这种可能不会发生</p><p>   2. raise ERROR if xxx 就是这个意思啊</p>"
    end
  end
end
